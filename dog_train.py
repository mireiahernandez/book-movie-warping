...
if __name__ == "__main__":
    ...
    parser.add_argument('--supervise', type=float, default=1., help='percentage of GT points to use')
    ...
    exp_name = f"DOG_{direction}_kernel_{kernel_type}_loss_{args.exp_info}_try_{args.try_num}_h1h2{args.h1}_{args.h2}"
    ...
    # Get feats
    image_feats = np.array(ImageOps.grayscale(Image.open('data/dog.jpeg'))).T
    text_feats = np.array(ImageOps.grayscale(Image.open('data/dog.jpeg'))).T
    ...
    supervise =  np.random.choice(len_input, int(len_input*args.supervise))
    # Begin training
    start_time = time.time()
    loss_prev = 0
    loss_now = 1000
    epoch = 0
    while epoch < 2000 : #abs(loss_prev - loss_now) > 1e-10 and epoch<500:
        ...
        lossGT = loss_gt(pred_invf_times_scaled[supervise], invf_times.to(device)[supervise] / (len_input - 1))
        ...
        wandb.log({'epoch': epoch,
                   ...
                   'ground_truth_loss': lossGT, 'lambda_gt': args.gt_loss, 'lambda rec': args.rec_loss})
        ...
        if epoch  % 500==0:
            ...
            # Get feats
            image_feats = np.array(ImageOps.grayscale(Image.open('data/dog.jpeg'))).T
            text_feats = np.array(ImageOps.grayscale(Image.open('data/dog.jpeg')).resize((275, 183 * (1+epoch//500)))).T
            # Transform to tensors and blur (optional)
            if blur and direction == 'm2b':
                kernel = np.array([[1, 4, 6, 4, 1]]) / 16
                text_feats = signal.convolve2d(text_feats.T, kernel, mode='valid', boundary='wrap')
